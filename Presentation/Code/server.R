...
shinyServer(function(input, output,session) {
 ...
 if(input$username!="" && input$password!=""){
  ...
  observeEvent(input$down, {
    ...
    if(input$nameit=='bar1'){
      png(filename="~/Desktop/namek2346.png")
      ...
      #plot(fit)
      dev.off()
    } else if(input$nameit=='bar2'){
      png(filename="~/Desktop/namek235.png")
    ...
    if (!isTRUE(USER$Logged)) {
      fluidRow(column(width=4, offset = 4,
                   ...
                   fluidPage(theme="css.css",
                             ...
                             # selectInput("nameit", "Choose Graph to plot:",
                             #             c("bar1" = "bar1",
                             #               "bar2" = "bar2")),
                             # 
                             # 
                                 ...
                                 fluidRow(
                                  ...
                                  actionButton("logout","LOGOUT"),
                                   ...
                                   tabsetPanel(id="navbar1",
                                               tabPanel("HOME",textOutput("home"),textOutput("valid"),align='center'),
                                               ...
                                               tabPanel("Explain Chat Graph",plotOutput("downloads")),
    ...
    sidebarLayout(
      sidebarPanel(width = 3,
        ...
        fluidRow(
          ...
          selectInput("nameit", "Choose Graph to download:",
                      c("bar1" = "bar1",
                        "bar2" = "bar2")),
  ...
  output$downloads<-renderPlot({
    ...
    rea
    barplot(rea$xx, las = 1, names.arg = rea$pe,col = brewer.pal(1, "Blues"), main ="Number of Chats Started between 00:00 to 23:60",ylab = "Number of Chats",xlab = "hours in 24 hour clock") 
    ...
    #                fluidRow(
    #                
    #                  selectInput("v_model","Download Model",choices=c("select download","PIE CHART","BAR GRAPH","WORDCLOUD","CHAT","BARGRAPH ANALYSIS","COUNTRY","TIME","SCATTER PLOT")),
    #                  downloadButton(outputId = "downn","Download")
    #                )
    #       
    #                
    #   ),
  ...
  output$kk <- renderPlot({
    ...
    if (input$chat == "WORDCLOUD") { 
     ...
     wordcloud(words = d$word, freq = d$freq, min.freq =20 , max.words=200, 
      ...
      rea<-data.frame(pe,xx)
      rea
      barplot(rea$xx, las = 1, names.arg = rea$pe, main ="BAR GRAPH SHOWING NUMBER OF ACTIVE CHATS ON DIFFERENT HOURS OF THE DAY ",ylab = "Average Number of Chat Sessions",xlab = "hours in 24 hour clock") 
